...
+ In the Dynamixel Wizard, you are able to set angle limits/ranges for the servos, this is indicated by the light gray circle. This can be useful to quickly see what the range of the servos are when they are attached to Poppy. Be aware, however, that these are **hard** limits. The servo will not be able to go past the limit no matter how much you want it to. The servo will go into overload mode when it is given a goal position past that limit, which of course requires a power cycle. Be careful to not position the servo beyond these limits when they are freely swinging, they will also go into overload when given a command. Despite these issues, if you have a small swinging path for the servo that does not cross the 0&deg; to 360&deg; boundary, then it is recommended to set these limits. Sometimes though, they may be more trouble than they are worth, so not every servo on Poppy has limits set. We set as many as we could and found were useful to do so.
